 ...
 * Model Preloader
 *
 * Download and verify model files before WebLLM loads them.
 * Files are stored in Cache API where WebLLM will find them.
 */
...
import { ModelVerificationError } from './types.js';
import {
  loadModelManifest,
  getModelEntry,
  getFileUrl,
...
/**
 * Cache names used by web-llm
 * These must match exactly for web-llm to find our cached files
 */
const WEBLLM_MODEL_CACHE = 'webllm/model';
const WEBLLM_CONFIG_CACHE = 'webllm/config';
const WEBLLM_WASM_CACHE = 'webllm/wasm';

/**
 * Get the appropriate cache name for a file type
 */
function getCacheNameForFile(filename: string): string {
  if (filename.endsWith('.wasm')) {
    return WEBLLM_WASM_CACHE;
  }
  if (filename === 'mlc-chat-config.json') {
    return WEBLLM_CONFIG_CACHE;
  }
  // Model weights, tokenizer, etc.
  return WEBLLM_MODEL_CACHE;
}
...
/**
 * Preload and verify a WebLLM model
 *
 * Downloads all model files, verifies their integrity, and stores them
 * in Cache API. WebLLM will find these cached files when loading.
 *
 * @param modelId - WebLLM model ID (e.g., "Phi-3-mini-4k-instruct-q4f16_1-MLC")
 ...
 *
 * const result = await preloadVerifiedModel('Phi-3-mini-4k-instruct-q4f16_1-MLC', {
...
export async function preloadVerifiedModel(
  ...
  for (const [filename, fileInfo] of files) {
    ...
    // Use the correct cache for this file type (matches web-llm's cache structure)
    const cacheName = options.cacheName ?? getCacheNameForFile(filename);
    ...
    if (cached) {
      filesComplete++;
      const size = fileInfo.size ?? (await cached.arrayBuffer()).byteLength;
      totalBytes += size;
    ...
    // Store in cache (web-llm will find it here)
...
async function downloadAndVerifyFile(
  ...
  try {
    ...
    if (onFail === 'warn') {
      console.warn(`[verifyfetch/webllm] Verification failed for ${filename}:`, error);
...
export async function clearModelCache(
  ...
  if (!modelId) {
    // Clear all web-llm caches
    if (options?.cacheName) {
      await caches.delete(options.cacheName);
    } else {
      await caches.delete(WEBLLM_MODEL_CACHE);
      await caches.delete(WEBLLM_CONFIG_CACHE);
      await caches.delete(WEBLLM_WASM_CACHE);
    }
    return;
  }
