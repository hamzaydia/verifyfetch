 ...
 *
 * Load, validate, and query verification manifests for Transformers.js models.
 */
 ...
 * @example
 * ```ts
 * const manifest = await loadModelManifest('/models/vf-hf.manifest.json');
 * ```
 */
...
/**
 * Resolve a URL from a Transformers.js fetch request to find its model and file
 * in the manifest. Handles HuggingFace Hub URL patterns.
 *
 * @returns The SRI hash if found, null otherwise
 */
export function lookupSriByUrl(
  url: string,
  manifest: ModelVerificationManifest
): { modelId: string; filename: string; fileInfo: ModelFileInfo } | null {
  const cleanUrl = stripUrlParams(url);

  // Try direct URL match against baseUrl + filename for each model
  for (const [modelId, entry] of Object.entries(manifest.models)) {
    ...
    for (const [filename, fileInfo] of Object.entries(entry.files)) {
      ...
      if (cleanUrl === fullUrl) {
        return { modelId, filename, fileInfo };
      }
    }
  }

  // Try matching by extracting model ID and filename from HF Hub URL patterns:
  // https://huggingface.co/{org}/{model}/resolve/{revision}/{path}
  const hfMatch = cleanUrl.match(
    /huggingface\.co\/([^/]+\/[^/]+)\/resolve\/[^/]+\/(.+)$/
  );

  if (hfMatch) {
    const [, urlModelId, urlPath] = hfMatch;
    // Check if model ID is in manifest
    const entry = manifest.models[urlModelId];
    if (entry) {
      // Check if path matches a file
      const fileInfo = entry.files[urlPath];
      if (fileInfo) {
        return { modelId: urlModelId, filename: urlPath, fileInfo };
      }
    }
  }
